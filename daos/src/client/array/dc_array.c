...
struct dc_array {
	...
	/** DAOS object handle */
	daos_handle_t		daos_oh;  //对应的daos object的handler
...
struct md_params {
	...
	d_sg_list_t		sgl;
	d_iov_t			sg_iov;
	uint64_t		md_vals[3]; //magic value、cell size、chunk size
};
...
dc_array_open(tse_task_t *task)
{
	...
	if (args->open_with_attr)  //用户已经指定了cell size和chunk size
	...
	/** Create task to open object */
	rc = daos_task_create(DAOS_OPC_OBJ_OPEN, tse_task2sched(task),   //完全本地化操作
			      0, NULL, &open_task);
	if (rc != 0) {
	...
	open_args->oh	= args->oh;//[指针]将open task获得的object handler写入task 的arg中。dc_object
	...
	if (args->open_with_attr) {
		...
		rc = tse_task_register_comp_cb(task, open_handle_cb, &args/*upper level task的arg*/,
					       sizeof(args));
		if (rc != 0) {
	...
	rc = tse_task_register_cbs(fetch_task, fetch_md_cb, &args/*upper level task的arg*/,
				   sizeof(args), NULL, NULL, 0);
	if (rc != 0) {
		D_ERROR("Failed to register prep CB\n");
		D_GOTO(err_put2, rc);
	    ...
	    daos_opc_t op_type, tse_task_t *task)
{
	...
	daos_off_t	array_idx; /* object array index of current range */ //就是array中某个record的index
	...
	array = array_hdl2ptr(array_oh);//handle->ptr，daos维护了一个handle->ptr的hash表
	if (array == NULL)
	 ...
	 */
	if (op_type == DAOS_OPC_ARRAY_READ && array->byte_array) { //创建get size 任务
	 ...
	 */
	while (u < rg_iod->arr_nr) {//u表示第i个range 
		daos_iod_t	*iod; // 1个DKey下的数据用以1个iod，1个iod中可能有多个record extents
		daos_iom_t	*iom;
		d_sg_list_t	*sgl;
		daos_key_t	*dkey;
		uint64_t	dkey_val;
		...
		struct io_params *params; //params for this dkey io 
		...
		//算出后三个参数
		//record_i：record在1个dkey chunk中的相对index
		//array_idx: record在整个object中的相对index(第i个)
		rc = compute_dkey(array, array_idx, &num_records, &record_i, //当前这个range的idx
				  &dkey_val);
		if (rc != 0) {
		 ...
		 */
		//这个循环处理完1个DKey下的所有record extent
		do {
			daos_off_t	old_array_idx;
			daos_recx_t	*new_recxs;  //1个IOD处理一个dkey下的array write，但是可能有多个record extent
			...
			iod->iod_recxs[i].rx_nr = (num_records > records) ?  //records可能跨越多个dkey，num_records一定是在一个dkey下
				records : num_records;
			 ...
			 * current dkey.
			 */
			if (records > num_records) {  //range跨越两个DKey的情况
				array_idx += num_records;
				records -= num_records;
				dkey_records += num_records;
				break;  //已经完成1个dkey的IOD了，break处理下一个dkey
			}
			...
			if (array_idx < old_array_idx + num_records &&  //下一个range的index和上一个range是在同一个dkey里
		...
		//在while循环中，即为每一个dkey下的update/fetch操作创建1个obj_update/obj_fetch子task
		...
		} else if (op_type == DAOS_OPC_ARRAY_WRITE ||   //写
			   op_type == DAOS_OPC_ARRAY_PUNCH) {
			daos_obj_update_t *io_arg;

			rc = daos_task_create(DAOS_OPC_OBJ_UPDATE,   //调用dc_obj_update_task
					      tse_task2sched(task),
					      0, NULL, &io_task);
			if (rc != 0) {
...
dc_array_write(tse_task_t *task)
{
	...
	return dc_array_io(args->oh, args->th, args->iod, args->sgl,//这里oh是array_handler，说明write array前已经获得array handle了
			   DAOS_OPC_ARRAY_WRITE, task);						//，也意味着获得了该array的一些信息，如shard列表等		
}
...
adjust_array_size_cb(tse_task_t *task, void *data)
{
		...
		if (props->size == 0 || dkey_val > props->dkey_val) {
			...
			if (props->record_i + 1 != props->chunk_size) {
				...
				D_DEBUG(DB_IO, "Punch extent in key "DF_U64"\n",
					dkey_val);
				rc = punch_extent(args->oh, args->th, dkey_val, props->record_i,  //连接server
	...
	if (props->update_dkey) {
		...
		rc = add_record(args->oh, args->th, props, &task_list);   //连接server
		if (rc)
			goto out;
	}
...
dc_array_set_size(tse_task_t *task)
{
	...
	rc = daos_task_create(DAOS_OPC_OBJ_LIST_DKEY, tse_task2sched(task),  //连接server，list dkey
			      0, NULL, &enum_task);
	if (rc)
		D_GOTO(err_task, rc);
