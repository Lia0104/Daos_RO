	  ...
	  size_t len, dfs_obj_t *file)
{
	...
	if (ocreat) {
		...
		rc = insert_entry(dfs->layout_v, parent->oh, DAOS_TX_NONE, file->name, len,  //写server obj
				  DAOS_COND_DKEY_INSERT, entry);
		if (rc == EEXIST && !oexcl) {
			/** just try fetching entry to open the file */
			daos_array_close(file->oh, NULL);//exist cancel为不为真，不返回，打开arr obj
		} else if (rc) {
			daos_array_close(file->oh, NULL);
			D_DEBUG(DB_TRACE, "Insert file entry %s failed (%d)\n", file->name, rc);
			return rc;  //exist cancel为真这里就直接返回
		} else {
			D_ASSERT(rc == 0);
			return 0;
		}
...
lookup_rel_path_loop:
		...
		entry.chunk_size = 0;
		rc = fetch_entry(dfs->layout_v, parent.oh, DAOS_TX_NONE, token, len, true,  //读元数据
