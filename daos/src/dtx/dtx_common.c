		 ...
		 struct dtx_memberships *mbs, struct dtx_leader_handle **p_dlh)
{
	...
	if (rc == 0 && sub_modification_cnt > 0)
		rc = vos_dtx_attach(dth, false, (flags & DTX_PREPARED) ? true : false);  //Generate DTX entry for the given DTX, and attach it to the DTX handle.
...
dtx_begin(daos_handle_t coh, struct dtx_id *dti,
	  struct dtx_epoch *epoch, uint16_t sub_modification_cnt,   //sub_modification_cnt==0
	  uint32_t pm_ver, daos_unit_oid_t *leader_oid,
		    ...
		    dtx_agg_cb_t agg_cb, void *agg_cb_arg, void *func_arg)
{
	 ...
	 */
	rc = ABT_future_create(dlh->dlh_normal_sub_cnt + 1, dtx_comp_cb, &dlh->dlh_future);  //argobots线程模型
	if (rc != ABT_SUCCESS) {
