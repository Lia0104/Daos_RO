...
//写lc attr：rdb_lc_replicas和rdb_lc_nreplicas
static int
rdb_raft_store_replicas(daos_handle_t lc, uint64_t index, const d_rank_list_t *replicas)
{
	...
	D_ASSERTF(replicas->rl_nr <= UINT8_MAX, "nreplicas = %u",
		  replicas->rl_nr);
	nreplicas = replicas->rl_nr;/** number of ranks */
...
rdb_raft_load_snapshot(struct rdb *db)
{
	...
	rc = raft_begin_load_snapshot(db->d_raft, db->d_lc_record.dlr_base_term,  //Start loading snapshot
	...
	for (i = 0; i < replicas->rl_nr; i++) {   //raft_begin_load_snapshot清除所有node，这里又添加node
...
static raft_cbs_t rdb_raft_cbs = {
	...
	.notify_membership_event	= rdb_raft_cb_notify_membership_event,  //111111
		   ...
		   struct rdb_lc_record *record)
{
	...
	rc = rdb_mc_update(mc, RDB_MC_ATTRS, 1 /* n */, key, &value); //更新RDB_MC_ATTRS下的rdb_mc_lc
	if (rc != 0) {
 ...
 * error.
 * 1. 创建log container: 写rdb_mc_lc，创建container
 * 2. 写log container中的replicas attr
 */
int
...
rdb_raft_load(struct rdb *db)
{
	...
	if (rc == 0) {
		rc = raft_set_current_term(db->d_raft, term);//更新raft server的term
		D_ASSERTF(rc == 0, DF_RC"\n", DP_RC(rc));
	...
	if (rc == 0) {
		rc = raft_vote_for_nodeid(db->d_raft, vote);  //Vote for a server.
		D_ASSERTF(rc == 0, DF_RC"\n", DP_RC(rc));
	} else if (rc != -DER_NONEXIST) {
		goto out;
	}
...
rdb_raft_start(struct rdb *db)
{
	...
	db->d_raft = raft_new();  //Initialise a new Raft server.
	if (db->d_raft == NULL) {
	...
	rc = rdb_raft_load(db); //Load raft persistent state, if any
	if (rc != 0) {
	...
	raft_set_election_timeout(db->d_raft, election_timeout); //选举时间间隔
