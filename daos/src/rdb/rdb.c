...
rdb_create(const char *path, const uuid_t uuid, size_t size,
	   const d_rank_list_t *replicas, struct rdb_cbs *cbs, void *arg,   //d_rank_t是server identification
	   struct rdb **dbp)
{
	daos_handle_t	pool;
	daos_handle_t	mc;
	...
	if (rc != 0)
		goto out_pool_hdl;
	rc = vos_cont_open(pool, (unsigned char *)uuid, &mc);  //metadata container
	if (rc != 0)
		goto out_pool_hdl;

	/* Initialize the layout version. */
	d_iov_set(&value, &version, sizeof(version));
	rc = rdb_mc_update(mc, RDB_MC_ATTRS, 1 /* n */, &rdb_mc_version,//初始version为1
			   &value);
	if (rc != 0)
		goto out_mc_hdl;
