	    ...
	    struct vos_obj_df **obj_p, struct vos_ts_set *ts_set)
{
	...
	*obj_p = NULL;
	d_iov_set(&key_iov, &oid, sizeof(oid));  //iov：iovec for memory buffer 
	d_iov_set(&val_iov, NULL, 0);

	rc = dbtree_fetch(cont->vc_btr_hdl, BTR_PROBE_EQ,   //[out]:val_iov
		  ...
		  struct vos_ts_set *ts_set)
{
	...
	rc = vos_oi_find(cont, oid, &obj, ts_set);  //Find an entry in the obj_index by @oid
	if (rc == 0)
		goto do_log;
	if (rc != -DER_NONEXIST)
		return rc;
	...
	rc = dbtree_upsert(cont->vc_btr_hdl, BTR_PROBE_EQ, DAOS_INTENT_DEFAULT,  //update or insert
