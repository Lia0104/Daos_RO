...
struct vos_io_context {
	...
	struct bio_desc		*ic_biod;  //就1个biod
	...
	struct vos_rsrvd_scm	*ic_rsrvd_scm;   //SCM extents
	...
	d_list_t		 ic_blk_exts;           //NVMe extent
...
vos_ioc_reserve_init(struct vos_io_context *ioc, struct dtx_handle *dth)
{
	...
	for (i = 0; i < ioc->ic_iod_nr; i++) {
		...
		total_acts += iod->iod_nr; //iod中extent的数量
	}

	D_ALLOC_ARRAY(ioc->ic_umoffs, total_acts);//申请total_acts个空间(每个空间是一个uint64用于存放offset)，用于update
	if (ioc->ic_umoffs == NULL)
		return -DER_NOMEM;

	if (vos_ioc2umm(ioc)->umm_ops->mo_reserve == NULL) //Reserve space with specified size.
		return 0;

	size = sizeof(*ioc->ic_rsrvd_scm) +    //指针解引用
		sizeof(struct pobj_action) * total_acts;   //pmdk tx 相关
	...
	if (!dtx_is_valid_handle(dth) || dth->dth_deferred == NULL) //simple test中dth为false，在这就返回
		return 0;
	       ...
	       struct vos_io_context **ioc_pp)
{
	...
	rc = vos_ioc_reserve_init(ioc, dth);  //分配scm空间。主要为填充 ic_rsrvd_scm、ic_umoffs等几个scm相关的字段
	if (rc != 0)
		goto error;
	...
	rc = vos_ts_set_allocate(&ioc->ic_ts_set, vos_flags, cflags, iod_nr,  // Allocate a timestamp set，simple test的ts set为null
				 dth);
	if (rc != 0)
		goto error;
	...
	bioc = cont->vc_pool->vp_io_ctxt;//bio_io_context
	D_ASSERT(bioc != NULL);
	...
	rc = dcs_csum_info_list_init(&ioc->ic_csum_list, iod_nr);  //Initialize memory for storing the csum_infos. 申请checksum的空间
	if (rc != 0)
		goto error;

	//这个循环bsgl是局部变量，最后好像没起作用？ 
	for (i = 0; i < iod_nr; i++) {  //遍历iod 列表
		int iov_nr = iods[i].iod_nr; //1个iod中extent的数量
		struct bio_sglist *bsgl;

		if ((iods[i].iod_type == DAOS_IOD_SINGLE && iov_nr != 1)) {  //出错
		...
		bsgl = bio_iod_sgl(ioc->ic_biod, i);  //Helper function to get the specified SG list of an io descriptor；获取biod中的第i个sg list
		rc = bio_sgl_init(bsgl, iov_nr);   //初始化sgl：设置iov数量，令iov=0
		if (rc != 0)
			goto error;
	}
...
iod_fetch(struct vos_io_context *ioc, struct bio_iov *biov)
{
	...
	bsgl = bio_iod_sgl(ioc->ic_biod, ioc->ic_sgl_at);//拿到biod中的sgl，对应一个vos iod
	D_ASSERT(bsgl != NULL);
	iov_nr = bsgl->bs_nr;
	...
	bsgl->bs_iovs[iov_at] = *biov;  //相当于地址？
	bsgl->bs_nr_out++;
	ioc->ic_iov_at++;
	return 0;
}
		  ...
		  daos_size_t *rsize, struct vos_io_context *ioc)
{
	...
	struct bio_iov		 biov; /* iov to return data buffer */  //相当于地址
	int			 rc;
...
dkey_fetch(struct vos_io_context *ioc, daos_key_t *dkey)
{
	...
	rc = obj_tree_init(obj);//obj_tree
	if (rc != 0)
		return rc;

	rc = key_tree_prepare(obj, obj->obj_toh/*object tree*/, VOS_BTR_DKEY,   //索引树
			      dkey, 0, DAOS_INTENT_DEFAULT, &krec,
			      &toh/*返回的结果*/, ioc->ic_ts_set);
...
fetch_akey:
	for (i = 0; i < ioc->ic_iod_nr; i++) {
		...
		rc = akey_fetch(ioc, toh);//toh：key_tree_prepare得到，在这里作为父树进行查找akey
		if (vos_dtx_continue_detect(rc))
			continue;
...
vos_fetch_begin(daos_handle_t coh, daos_unit_oid_t oid, daos_epoch_t epoch,
		daos_key_t *dkey, unsigned int iod_nr/*iods中的iod个数*/,
		...
		struct dtx_handle *dth)
{
	...
	rc = vos_obj_hold(vos_obj_cache_current(), ioc->ic_cont, oid,   //有进cache,找到到obj的引用
			  &ioc->ic_epr, ioc->ic_bound, VOS_OBJ_VISIBLE,
			  DAOS_INTENT_DEFAULT, &ioc->ic_obj, ioc->ic_ts_set);  //rc==0的话不进下面的if
	if (stop_check(ioc, VOS_COND_FETCH_MASK | VOS_OF_COND_PER_AKEY, NULL,
		       &rc, false)) { 
		if (rc == 0) {
			if (ioc->ic_read_ts_only)
...
fetch_dkey: //拿到obj下dkey
	...
	rc = dkey_fetch(ioc, dkey); //访问vos tree
	if (rc != 0)
		goto out;
set_ioc:
	*ioh = vos_ioc2ioh(ioc);
		   ...
		   uint16_t minor_epc)
{
	...
	umoff = iod_update_umoff(ioc);  //从ioc中拿umoff
	    ...
	    uint16_t minor_epc)
{
	...
	rc = key_tree_prepare(obj, ak_toh/*akey树*/, VOS_BTR_AKEY,
			      &iod->iod_name/*akey*/, flags, DAOS_INTENT_UPDATE,
			      &krec, &toh/*子树的handle，这里为0(空)*/, ioc->ic_ts_set);
	if (rc < 0) {
	    ...
	    uint16_t minor_epc)
{
	...
	rc = key_tree_prepare(obj, obj->obj_toh, VOS_BTR_DKEY, dkey,  //准备好krec_df(dkey_tree)和SVT或EVT(akey_tree)
	...
	for (i = 0; i < ioc->ic_iod_nr; i++) {
		iod_set_cursor(ioc, i);  //sgl_at和iov_at
		...
		daos_size_t size)
{
	...
	if (vos_cont2umm(cont)->umm_ops->mo_reserve != NULL) {
		...
		umoff = umem_reserve(vos_cont2umm(cont), act, size);  //Reserve SCM through umem_reserve() for a PMDK transaction
		   ...
		   daos_size_t size, enum vos_io_stream ios, uint64_t *off)
{
	...
	vsi = vos_cont2pool(cont)->vp_vea_info;  //Versioned Block Allocator
	D_ASSERT(vsi);

	hint_ctxt = cont->vc_hint_ctxt[ios];  //io stream
	D_ASSERT(hint_ctxt);

	blk_cnt = vos_byte2blkcnt(size);

	rc = vea_reserve(vsi, blk_cnt, hint_ctxt, rsrvd_nvme);  //预留地址加入rsrvd_nvme(list)尾部
	if (rc)
		return rc;
	...
	*off = ext->vre_blk_off << VOS_BLK_SHIFT;  /// Start block offset of the reserved extent */
	return 0;
}
	      ...
	      uint64_t *off)
{
	...
	if (media == DAOS_MEDIA_SCM) {   //scm预留空间
		umem_off_t	umoff;  //The offset of an object from the base address of the pool
		...
		if (!UMOFF_IS_NULL(umoff)) {
			...
			ioc->ic_umoffs_cnt++;
			*off = umoff;
			return 0;   //scm空间预留成功后 在此返回
		}

		now = daos_gettime_coarse();
		if (now - ioc->ic_cont->vc_io_nospc_ts > VOS_NOSPC_ERROR_INTVL) {  //出错
			daos_size_t	scm_used = 0, scm_active = 0;

			rc = pmemobj_ctl_get(vos_cont2pool(ioc->ic_cont)->vp_umm.umm_pool,   //获取pool的相关参数
	...
	D_ASSERT(media == DAOS_MEDIA_NVME);     //NVMe预留空间
...
iod_reserve(struct vos_io_context *ioc, struct bio_iov *biov)
{
	...
	bsgl = bio_iod_sgl(ioc->ic_biod, ioc->ic_sgl_at);  //Helper function to get the specified SG list of an io descriptor
	D_ASSERT(bsgl->bs_nr != 0);
	D_ASSERT(bsgl->bs_nr > bsgl->bs_nr_out);
	D_ASSERT(bsgl->bs_nr > ioc->ic_iov_at);

	bsgl->bs_iovs[ioc->ic_iov_at] = *biov;   //bsgl是biod中的一个元素的引用
	ioc->ic_iov_at++;
	bsgl->bs_nr_out++;
		   ...
		   daos_size_t size)
{
	...
	uint64_t		 off = 0;
	int			 rc;
	struct dcs_csum_info	*value_csum = vos_csum_at(ioc->iod_csums, ioc->ic_sgl_at);//一个iod对应一个checksum
	 ...
	 */
	scm_size = (media == DAOS_MEDIA_SCM) ?     //元数据大小+record大小，也就是1个irec的大小
		vos_recx2irec_size(size, value_csum) :
		vos_recx2irec_size(0, value_csum);

	rc = reserve_space(ioc, DAOS_MEDIA_SCM, scm_size, &off);  //返回off，ic_umoffs_cnt++，写umoffs数组的最后一个元素off
	if (rc) {
	...
	umoff = ioc->ic_umoffs[ioc->ic_umoffs_cnt - 1]; // ic_umoffs的最后一个元素
	irec = (struct vos_irec_df *)umem_off2ptr(vos_ioc2umm(ioc), umoff); //也就是说这个irec的空间在上面reserve_space函数中才申请的
	vos_irec_init_csum(irec, value_csum);   //初始化irec的csum
	...
	if (media == DAOS_MEDIA_SCM) {
		...
		off = umoff + (payload_addr - (char *)irec);//irec是umoff转化来的指针；计算出来payload_addr的偏移量off
	} else {
	...
	bio_addr_set(&biov.bi_addr, media, off);  //bio
	bio_iov_set_len(&biov, size);  //处理好sglist的一个biov
	rc = iod_reserve(ioc, &biov);  //biov赋值给ioc中的biod中的sglist[sglist_at]中的bio_vec[vec_at]
...
akey_update_begin(struct vos_io_context *ioc)
{
	struct dcs_csum_info	*iod_csums = vos_csum_at(ioc->iod_csums, ioc->ic_sgl_at);
	struct dcs_csum_info	*recx_csum;
	daos_iod_t *iod = &ioc->ic_iods[ioc->ic_sgl_at];  //iod
	int i, rc;

	if (iod->iod_type == DAOS_IOD_SINGLE && iod->iod_nr != 1) {  //出错
		D_ERROR("Invalid sv iod_nr=%d\n", iod->iod_nr);
		return -DER_IO_INVAL;
	}

	for (i = 0; i < iod->iod_nr; i++) {  //遍历iod中的iod_nr个extent
		daos_size_t size;
		uint16_t media;

		size = (iod->iod_type == DAOS_IOD_SINGLE) ? iod->iod_size :   //单条data的大小
				iod->iod_recxs[i].rx_nr * iod->iod_size;//某个extent有rx_nr个record，乘以每个record大小

		media = vos_policy_media_select(vos_cont2pool(ioc->ic_cont),  // Object placement policy
					 iod->iod_type, size, VOS_IOS_GENERIC);

		if (iod->iod_type == DAOS_IOD_SINGLE) {  // one indivisible value 
			rc = vos_reserve_single(ioc, media, size);  //为single value record 在SCM/NVMe上预留空间
		} else {
...
dkey_update_begin(struct vos_io_context *ioc)
{
	...
	for (i = 0; i < ioc->ic_iod_nr; i++) {  //遍历biod中的ic_iod_nr个SGLs，
		iod_set_cursor(ioc, i);   //第i个bsgl，对应于第i个vos iod
		 ...
		 uint32_t dedup_th, daos_handle_t *ioh, struct dtx_handle *dth)
{
	...
	rc = vos_check_akeys(iod_nr, iods);  //防止iods中存在多个iod的akey相同的情况
	if (rc != 0) {
		D_ERROR("Detected duplicate akeys, operation not allowed\n");
		return rc;
	}

	rc = vos_ioc_create(coh, oid, false, epoch, iod_nr, iods, iods_csums,   //创建IO上下文 [out]：ioc
			    flags, NULL, dedup_th, dth, &ioc);
	if (rc != 0)
		return rc;

	/* flags may have VOS_OF_CRIT to skip sys/held checks here */
	rc = vos_space_hold(vos_cont2pool(ioc->ic_cont), flags, dkey, iod_nr,     //计算本次update需要的scm/nvme空间，并检查剩余空间是否足够
...
vos_iod_sgl_at(daos_handle_t ioh, unsigned int idx)
{
	...
	return bio_iod_sgl(ioc->ic_biod, idx);  //Helper function to get the specified SG list of an io descriptor
}
	     ...
	     unsigned int sgl_nr)
{
	...
	rc = bio_iod_prep(ioc->ic_biod, BIO_CHK_TYPE_IO, NULL, 0);  //Prepare all the SG lists of an io descriptor.
	if (rc)
		return rc;

	rc = bio_iod_copy(ioc->ic_biod, sgls, sgl_nr);  //Helper function to copy data between SG lists of io descriptor and user specified DRAM SG lists.
		  ...
		  struct dtx_handle *dth)
{
	...
	rc = vos_update_begin(coh, oid, epoch, flags, dkey, iod_nr, iods,  //[out]:ioh
	...
	if (sgls) {
		rc = vos_obj_copy(vos_ioh2ioc(ioh), sgls, iod_nr);  //copy data between SG lists of io descriptor and user specified DRAM SG lists.
		if (rc)
