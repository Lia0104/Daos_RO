...
vos_space_query(struct vos_pool *pool, struct vos_pool_space *vps, bool slow)
{
	...
	SCM_SYS(vps) = POOL_SCM_SYS(pool);  //保留的系统空间（用于空间回收、重建等）（以字节为单位）
	NVME_SYS(vps) = POOL_NVME_SYS(pool);

	/* Query SCM used space */
	rc = pmemobj_ctl_get(pool->vp_umm.umm_pool,  //pmdk
	       ...
	       struct dcs_iod_csums *iods_csums, daos_size_t *space_hld)
{
	...
	rc = vos_space_query(pool, &vps, false);//获取pool的空间信息
	if (rc) {
	...
	estimate_space(pool, dkey, iod_nr, iods, iods_csums, &space_est[0]);  //Estimate how much space will be consumed by an update request. 
	...
	if (scm_left < SCM_SYS(&vps))//SCM_SYS:scm的保留空间
		goto error;

	scm_left -= SCM_SYS(&vps);
	if (scm_left < POOL_SCM_HELD(pool))//POOL_SCM_HELD：为正在进行的update保留的空间
		goto error;

	scm_left -= POOL_SCM_HELD(pool);
	if (scm_left < space_est[DAOS_MEDIA_SCM]) //space_est：上面estimate_space评估的本次update所需的空间
		goto error;
	...
	if (nvme_left < NVME_SYS(&vps)) //NVME_SYS:NVMe的保留空间
		goto error;

	nvme_left -= NVME_SYS(&vps);
	/* 'NVMe held' has already been excluded from 'NVMe free' */

	if (nvme_left < space_est[DAOS_MEDIA_NVME])  //预估所需要的空间
		goto error;
