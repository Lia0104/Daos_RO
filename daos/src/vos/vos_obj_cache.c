		     ...
		     struct vos_object **objp)
{
	...
	rc = vos_obj_hold(occ, cont, oid, &epr, 0, VOS_OBJ_DISCARD, //VOS_OBJ_DISCARD表示获得obj是为了discard，所以要与create等互斥
	...
	obj->obj_discard = true;  //在这里discard
	*objp = obj;
...
{
	struct vos_object	*obj; //cache object
	struct daos_llink	*lret;
	struct obj_lru_key	 lkey;  //包含object id和container id
	int			 rc = 0;
	int			 tmprc;
	...
	*obj_p = NULL;
	...
	if (rc == -DER_NONEXIST) {
		...
		/** Object is in cache */
		obj = container_of(lret, struct vos_object, obj_llink);//obj等于lret所在的vos_object的指针。
	}
	...
	if (intent == DAOS_INTENT_KILL) {
		if (obj != &obj_local) {
			if (vos_obj_refcount(obj) > 2)  //obj被引用大于2，即还有其他程序正在使用该obj
				D_GOTO(failed, rc = -DER_BUSY);

			vos_obj_evict(occ, obj); //evict
		}
	...
	if (obj->obj_df) {
		...
		if (create || intent == DAOS_INTENT_PUNCH)
			vos_ilog_ts_ignore(vos_obj2umm(obj),//缓存中有obj且缓存了pmem地址，就不用再log create了？
					   &obj->obj_df->vo_ilog);
	...
	if (!create) {
		rc = vos_oi_find(cont, oid, &obj->obj_df, ts_set); //在object index中查找oid对应的obj的pmem地址
		if (rc == -DER_NONEXIST) {
...
check_object:
	if (obj->obj_discard && (create || (flags & VOS_OBJ_DISCARD) != 0)) { //obj已经是discard，但是又要1.create或2.discard，就出错。
	...
	if (!create) {
		rc = vos_ilog_fetch(vos_cont2umm(cont), vos_cont2hdl(cont),  //获取ilog
