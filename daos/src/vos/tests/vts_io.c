...
vts_key_gen(char *dest, size_t len, bool is_dkey, struct io_test_args *arg)
{
	...
	} else if (arg->ta_flags & TF_FIXED_AKEY) {  //simple test中ta_flags不等于TF_FIXED_AKEY
...
io_test_obj_update(struct io_test_args *arg, daos_epoch_t epoch, uint64_t flags,
		   daos_key_t *dkey, daos_iod_t *iod, d_sg_list_t *sgl,  //iod / sgl
		   struct dtx_handle *dth, bool verbose)
{
	...
	use_checksums = arg->ta_flags & TF_USE_CSUMS || g_force_checksum;//simple test每走checksum
	...
	if (!(arg->ta_flags & TF_ZERO_COPY)) {  //不是ZERO_COPY走这
	...
	rc = vos_update_begin(arg->ctx.tc_co_hdl, arg->oid, epoch, flags, dkey,
			      1, iod, iod_csums, 0, &ioh, dth);  //iod_nr == 1;
	if (rc != 0) {
	...
	rc = bio_iod_prep(vos_ioh2desc(ioh), BIO_CHK_TYPE_IO, NULL, 0); //biod，Prepare all the SG lists of an io descriptor,与非zc的区别在这。
	if (rc)
		goto end;
	...
	rc = bio_iod_copy(vos_ioh2desc(ioh), sgl, 1);  //copy data between SG lists of io descriptor and user specified DRAM SG lists.与非zc的区别在这。
	assert_rc_equal(rc, 0);
	/*
...
/**
 * 2个数据结构：sg list 、 iov 、 iod
 * 
*/
static int
io_update_and_fetch_dkey(struct io_test_args *arg, daos_epoch_t update_epoch,   // arg,1,1
			 daos_epoch_t fetch_epoch)
{
	...
	if (arg->ta_flags & TF_REC_EXT) {  //extent
	...
	if (!(arg->ta_flags & TF_PUNCH)) {  //不是punch
		...
		set_iov(&dkey, &dkey_buf[0], arg->ofeat & DAOS_OF_DKEY_UINT64); //dkey是iov，设置iov->buf和iov—>len
		set_iov(&akey, &akey_buf[0], arg->ofeat & DAOS_OF_AKEY_UINT64);

		dts_buf_render(update_buf, UPDATE_BUF_SIZE); //Fill in readable random bytes into the buffer 
		d_iov_set(&val_iov, &update_buf[0], UPDATE_BUF_SIZE);
		iod.iod_size = recx_size;//value或record的大小
		rex.rx_nr    = recx_nr;
	} else {
...
static void
io_oi_test(void **state)  //object index
{
	struct io_test_args	*arg = *state;
	...
	rc = umem_tx_begin(vos_cont2umm(cont), NULL);  //地层调用pmdk pmemobj_tx_begin
	assert_rc_equal(rc, 0);

	//查找oid的object，没找到就创建object
	//返回该object的ptr
...
io_obj_cache_test(void **state)
{
	...
	struct umem_instance	*ummg;  //umm global?
	struct umem_instance	*umml;  //umm local
	...
	rc = vos_obj_cache_create(10, &occ);  //2^10 
	assert_rc_equal(rc, 0);
	...
	rc = vos_pool_create(po_name, pool_uuid, VPOOL_16M, 0, 0, &l_poh);  //创建并打开poh scm_sz=16m nvme_sz=0
	assert_rc_equal(rc, 0);

	rc = vos_cont_create(l_poh, ctx->tc_co_uuid);
	assert_rc_equal(rc, 0);

	rc = vos_cont_open(l_poh, ctx->tc_co_uuid, &l_coh); //这边打开的是l_coh不是ctx->tc_co_hdl
	assert_rc_equal(rc, 0);
	...
	rc = umem_tx_begin(ummg, NULL);  //底层调用pmemobj_tx_begin
	assert_rc_equal(rc, 0);

	rc = vos_obj_hold(occ, vos_hdl2cont(ctx->tc_co_hdl), oids[0], &epr, 0,   //从cache中找object并返回引用，如果找不到到PMEM中找
			  VOS_OBJ_CREATE | VOS_OBJ_VISIBLE, DAOS_INTENT_DEFAULT,
			  &objs[0], 0);
	assert_rc_equal(rc, 0);

	rc = vos_obj_discard_hold(occ, vos_hdl2cont(ctx->tc_co_hdl), oids[0], &obj1);  //Hold object for range discard
	assert_rc_equal(rc, 0);
	/** Should be prevented because object olready held for discard */
	rc = vos_obj_discard_hold(occ, vos_hdl2cont(ctx->tc_co_hdl), oids[0], &obj2);//已经discard和discard冲突，出错，所以obj2==null
	assert_rc_equal(rc, -DER_UPDATE_AGAIN);
	/** Should prevent simultaneous hold for create as well */
	rc = vos_obj_hold(occ, vos_hdl2cont(ctx->tc_co_hdl), oids[0], &epr, 0,  //discard和create冲突，出错
					   VOS_OBJ_CREATE | VOS_OBJ_VISIBLE, DAOS_INTENT_DEFAULT,
					   &obj2, 0);
	assert_rc_equal(rc, -DER_UPDATE_AGAIN);

	/** Need to be able to hold for read though or iteration won't work */
	rc = vos_obj_hold(occ, vos_hdl2cont(ctx->tc_co_hdl), oids[0], &epr, 0,  //这是可以获得obj的
	...
	rc = umem_tx_begin(umml, NULL);  //上面是ummg，这是umml
	assert_rc_equal(rc, 0);

	rc = vos_obj_hold(occ, vos_hdl2cont(l_coh), oids[1], &epr, 0,   //这里在l_coh创建oid[1]对应的cache obj？
			  VOS_OBJ_CREATE | VOS_OBJ_VISIBLE, DAOS_INTENT_DEFAULT,
			  &objs[0], 0);
	assert_rc_equal(rc, 0);
	vos_obj_release(occ, objs[0], false);
	...
	rc = hold_objects(objs, occ, &ctx->tc_co_hdl, &oids[1], 10, 15, true,  //找不到oids[1]对应的obj
			  -DER_NONEXIST);
	assert_int_equal(rc, 0);

	rc = hold_objects(objs, occ, &l_coh, &oids[1], 10, 15, true, 0); //找得到oids[1]对应的obj，区别：conh不同
	assert_int_equal(rc, 0);
	...
	rc = hold_objects(objs, occ, &l_coh, &oids[1], 15, 20, true, 0);//这里还能找到obj，说明release只是解除引用与obj的关系没有真正删除obj
	assert_int_equal(rc, 0);
...
io_simple_one_key_test(void **state, unsigned int flags)
{
	...
	arg->ta_flags = flags;
	rc = io_update_and_fetch_dkey(arg, 1, 1);//update/fetch epoch = 1
	assert_rc_equal(rc, 0);
}
