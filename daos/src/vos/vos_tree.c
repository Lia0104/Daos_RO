		 ...
		 struct vos_krec_df *krec, bool created, daos_handle_t *sub_toh)
{
	...
	if (krec->kr_bmap & unexpected_flag) {  //出错
	...
	if (krec->kr_bmap & expected_flag) {  //open，表示已经有sub tree？
		if (flags & SUBTR_EVT) {   //ev tree
			rc = evt_open(&krec->kr_evt, uma, &cbs, sub_toh);
		} else {
			rc = dbtree_open_inplace_ex(&krec->kr_btr, uma, coh,  //btree
						    pool, sub_toh);
		}
		if (rc != 0)
	...
	if (!created) {
		rc = umem_tx_add_ptr(vos_obj2umm(obj), krec,   //将可以访问的内存区域(krec)添加到
				     sizeof(*krec));
		if (rc != 0) {
...
key_tree_prepare(struct vos_object *obj, daos_handle_t toh,
		 ...
		 daos_handle_t *sub_toh, struct vos_ts_set *ts_set)  //sub_toh = akey_toh
{
	 ...
	 *   there.
	 */
	rc = dbtree_fetch(toh, BTR_PROBE_EQ, intent, key,//在obj tree中找到key的krec_df
			  NULL, &riov);
	switch (rc) {
	default:
	...
	case 0:
		krec = rbund.rb_krec;
		ilog = &krec->kr_ilog;
		/** fall through to cache re-cache entry ，没有break的意思*/
	case -DER_NONEXIST:
	...
	if (sub_toh) {  //sub_toh是上一级函数创建的指针
		D_ASSERT(krec != NULL);
		rc = tree_open_create(obj, tclass, flags, krec, created,  //返回akey tree的handle
				      sub_toh);
	}
