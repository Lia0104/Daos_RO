 ...
 * 找到pool map的pm地址，和pool map version
 */
static int
...
//Generate path to a target file
static char *
		   ...
		   const uint32_t *domains)
{
	...
	attr.dsa_order = 16;
	rc = rdb_tx_create_kvs(tx, kvs, &ds_pool_prop_handles, &attr); //创建新的kvs，value为空
	if (rc != 0) {
			...
			daos_prop_t *prop, d_rank_list_t *svc_addrs)
{
	...
	rc = d_backoff_seq_init(&backoff_seq, 0 /* nzeros */, 16 /* factor */,   //指数退避算法的序列生成器，在重试失败操作时生成一系列等待时间
...
pool_svc_alloc_cb(d_iov_t *id, struct ds_rsvc **rsvc)
{
	...
	if (rc != 0)
		goto err_lock;
	rc = rdb_path_push(&svc->ps_root, &rdb_path_root_key);//path是key的列表，push指在列表最后增加一个key
	if (rc != 0)
		goto err_root;
...
ds_pool_create_handler(crt_rpc_t *rpc)
{
	...
	d_iov_set(&value, NULL /* buf */, 0 /* size */);
	rc = rdb_tx_lookup(&tx, &svc->ps_root, &ds_pool_prop_map_buffer,//找map_buf
			   &value);
	if (rc != -DER_NONEXIST) {  //已经存在
		if (rc == 0)
	...
	attr.dsa_class = RDB_KVS_GENERIC;
	attr.dsa_order = 8;
	rc = rdb_tx_create_root(&tx, &attr);  //创建root就是初始化DB
	if (rc != 0)
		D_GOTO(out_tx, rc);
...
ds_pool_connect_handler(crt_rpc_t *rpc, int handler_version)
{
	...
	rc = rdb_tx_update(&tx, &svc->ps_handles, &key, &value);  //更新handler
	if (rc != 0)
		D_GOTO(out_map_version, rc);
