 ...
 * the MS.
 * 找到pool service的一个副本节点，作为rpc连接的end point
 */
int
 ...
 * 
 * daos_pool_info_t *info：pool相关信息，包括target数量、node数量、坏的target数量、raft leader、pool的空间使用情况等
 * 
 */
static int
		    ...
		    bool connect)
{
	...
	//获取该pool的engine rank 集合
	if (ranks != NULL) {
		d_rank_range_list_t	*range_list;
		bool	get_enabled = (info ? ((info->pi_bits & DPI_ENGINES_ENABLED) != 0) : false); //true返回all target enabled的storage engine，false相反
 ...
 */
/**
 * dc_pool *pool: client pool句柄
 * crt_endpoint_t *ep: cart endpoint
 * 
*/
static int
...
pool_connect_cp(tse_task_t *task, void *data)
{
	...
	struct dc_pool		  *pool = dc_task_get_priv(task); //Client pool handle
	...
	struct pool_connect_v5_out *pco = crt_reply_get(arg->rpc);//返回rpc reply buffer
	bool			   put_pool = true;
	int			   rc = task->dt_result;

	//client完成rpc，执行pool_rsvc_client_complete_rpc对rpc返回的结果进行分析
	//pool_rsvc_client_complete_rpc(struct dc_pool *pool, const crt_endpoint_t *ep, int rc_crt, struct pool_op_out *out, tse_task_t *task)
	...
	rc = pco->pco_op.po_rc;
	if (rc == -DER_TRUNC) { //trunc截断：client buffer太小？
	...
	//上面rpc连接没问题，process_query_reply处理server的回复数据，并包装在info中
			 ...
			 const char *label, daos_handle_t *poh)
{
	...
	// 从pool的副本中，即raft组中找一个节点作为rpc的连接点
	ep.ep_grp = pool->dp_sys->sy_group;
	rc = dc_pool_choose_svc_rank(label, pool->dp_pool, &pool->dp_client, // 为pool选择数据复制服务？？
	...
	rc = tse_task_register_comp_cb(task, pool_connect_cp, &con_args,   //tse：task执行引擎 cb:call back
				       sizeof(con_args));
	if (rc != 0)
		D_GOTO(out_bulk, rc);
...
dc_pool_connect(tse_task_t *task)
{
	...
	struct dc_pool		*pool = NULL;  //client pool handle
	const char		*label;
	uuid_t			 uuid;
	int			 rc;

	args = dc_task_get_args(task);
	pool = dc_task_get_priv(task); //以结构化形式task中的dt_private char数组
