...
bulk_map_one(struct bio_desc *biod, struct bio_iov *biov, void *data)
{
	...
	dma_biov2pg(biov, &off, &end, &pg_cnt, &pg_off);//biov这段连续空间转换成page相关
	...
	hdl = bulk_get_hdl(biod, biov, roundup_pgs(pg_cnt), pg_off, arg); //获取bulk handle
	if (hdl == NULL) {
		if (biod->bd_retry)
			return -DER_AGAIN;
	...
	bio_iov_set_raw_buf(biov, bulk_hdl2addr(hdl, pg_off));//设置biov->buf
