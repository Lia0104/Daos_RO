...
ut_device(void **state)
{
	...
	rc = smd_dev_add_tgt(dev_id1, 0);  //err
	...
	rc = smd_dev_add_tgt(dev_id2, 1);  //err
	assert_rc_equal(rc, -DER_EXIST);
...
ut_pool(void **state)
{
	...
	rc = smd_pool_add_tgt(id1, 0, 5000, 100);  //Dup target 0, idx: 0
	assert_rc_equal(rc, -DER_EXIST);

	rc = smd_pool_add_tgt(id1, 4, 4 << 10, 200);  //dismatch
	...
	rc = smd_pool_get_info(id3, &pool_info);  //err
	assert_rc_equal(rc, -DER_NONEXIST);
	...
	rc = smd_pool_get_blob(id1, 5, &blob_id); //err
	assert_rc_equal(rc, -DER_NONEXIST);
...
ut_dev_replace(void **state)
{
	...
	rc = smd_dev_replace(dev_id1, dev_id3, &pool_list);  //err
	assert_rc_equal(rc, -DER_INVAL);
	...
	rc = smd_dev_replace(dev_id1, dev_id2, &pool_list);  //err
	assert_rc_equal(rc, -DER_INVAL);
