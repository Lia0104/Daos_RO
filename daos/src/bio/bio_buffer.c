...
dma_alloc_chunk(unsigned int cnt)
{
	...
	if (bio_spdk_inited) {
		chunk->bdc_ptr = spdk_dma_malloc_socket(bytes, BIO_DMA_PAGE_SZ, NULL,
							bio_numa_node);  //SOCKET ID指定要分配DMA buffer的内存位置
	} else {
	      ...
	      unsigned int type)
{
	...
	D_ALLOC(biod, offsetof(struct bio_desc, bd_sgls[sgl_cnt]));//算上sgl_cnt大小的数组后biod的大小。
	if (biod == NULL)
		return NULL;
...
static int
copy_one(struct bio_desc *biod, struct bio_iov *biov, void *data) //biov是biod中一个sglist的biov。data包含dram sglist 数组的信息
{
	...
	sgl = &arg->ca_sgls[arg->ca_sgl_idx];  //DRAM sg list

	while (arg->ca_iov_idx < sgl->sg_nr) { //遍历dram sglist的所有iov
		d_iov_t *iov;
		ssize_t nob, buf_len;

		iov = &sgl->sg_iovs[arg->ca_iov_idx];
		buf_len = (biod->bd_type == BIO_IOD_TYPE_UPDATE) ?  //dram的buf len
		...
		nob = min(size, buf_len - arg->ca_iov_off);//比较biov中req data和dram iov中buf剩余空间的大小，小的为传输数据的大小
		if (arg->ca_size_tot) {
		...
		if (addr != NULL) {
			D_DEBUG(DB_TRACE, "bio copy %p size %zd\n",
				addr, nob);
			bio_memcpy(biod, media, addr, iov->iov_buf +   //对media和dram两个addr的空间数据进行复制，大小为nob；addr是biov->buf，即dma buffer的地址
					arg->ca_iov_off, nob);
			addr += nob;
		} else {
			/* fetch on hole */
	     ...
	     void *data)
{
	...
	for (i = 0; i < biod->bd_sgl_cnt; i++) {  //变量bio中的每个sglist
		struct bio_sglist *bsgl = &biod->bd_sgls[i];

		if (data != NULL) {  //simple test中arg为null
			if (cb_fn == copy_one) {
				struct bio_copy_args *arg = data;
		...
		for (j = 0; j < bsgl->bs_nr_out; j++) {
			...
			rc = cb_fn(biod, biov, data); //data是dram的sglist的相关信息
			if (rc)
				break;
		}
		if (rc)
...
dma_map_one(struct bio_desc *biod, struct bio_iov *biov, void *arg)
{
	...
	if (direct_scm_access(biod, biov)) {   //判断是否是direct acc SCM
		struct umem_instance *umem = biod->bd_ctxt->bic_umem;
		bio_iov_set_raw_buf(biov,				 // 将SCM的direct memory address写入biov的bi_buf
				    umem_off2ptr(umem, bio_iov2raw_off(biov)));
		return 0;
	}
	D_ASSERT(!BIO_ADDR_IS_DEDUP(&biov->bi_addr));

	bdb = iod_dma_buf(biod);  //Per-xstream DMA buffer
	dma_biov2pg(biov, &off, &end, &pg_cnt, &pg_off);  //biov起始偏移量和终止偏移量、可以划分的page数量和page偏移量
	 ...
	 */
	if (pg_cnt > bio_chk_sz) {  //page数量大于1个chunk
		chk = dma_alloc_chunk(pg_cnt);   //SPDK
		if (chk == NULL)
			return -DER_NOMEM;

		chk->bdc_type = biod->bd_chk_type;
		rc = iod_add_chunk(biod, chk);  //把chunk指针添加到biod的chunk数组中
		if (rc) {
			dma_free_chunk(chk);
			return rc;
		}
	...
	if (chk != NULL) {
		D_ASSERT(biod->bd_chk_type == chk->bdc_type);
		chk_pg_idx = chk->bdc_pg_idx;  //未被使用的page的偏移量
...
add_region:
	return iod_add_region(biod, chk, chk_pg_idx, chk_off, off, end,   //将region添加到 Reserved DMA buffer
			      bio_iov2media(biov));
}
...
void
bio_memcpy(struct bio_desc *biod, uint16_t media, void *media_addr, //media是biov->buf,即dma buffer地址，addr是dram sglig中iov的地址
	   void *addr, ssize_t n)
{
	struct umem_instance *umem = biod->bd_ctxt->bic_umem;

	D_ASSERT(biod->bd_type < BIO_IOD_TYPE_GETBUF);
	if (biod->bd_type == BIO_IOD_TYPE_UPDATE && media == DAOS_MEDIA_SCM) {  //对scm进行写
		/*
		...
		pmemobj_memcpy_persist(umem->umm_pool, media_addr, addr, n);//pmemobj_memcpy_persist(PMEMobjpool *pop, void *dest,	const void *src, size_t len)
	} else {
		if (biod->bd_type == BIO_IOD_TYPE_UPDATE)  //nvme 写
			memcpy(media_addr, addr, n);
		else
			memcpy(addr, media_addr, n);  //scm/nvme读
	}
}
...
nvme_rw(struct bio_desc *biod, struct bio_rsrvd_region *rg)
{
	...
	while (pg_cnt > 0) {
		/* NVMe poll needs be scheduled */  //轮询
		...
		if (biod->bd_type == BIO_IOD_TYPE_UPDATE)
			spdk_blob_io_write(blob, channel, payload,
					   page2io_unit(biod->bd_ctxt, pg_idx),//Offset is in io units from the beginning of the blob.
					   page2io_unit(biod->bd_ctxt, rw_cnt),//Size of data in io units.
...
dma_rw(struct bio_desc *biod)
{
	...
	for (i = 0; i < rsrvd_dma->brd_rg_cnt; i++) {  //遍历dma buffer中的每一个region
...
retry:
	rc = iterate_biov(biod, arg ? bulk_map_one : dma_map_one, arg);  //dma_map_one： Convert offset of @biov into memory pointer 
	if (rc) {
		/*
...
/**
 * 1. 对于direct acc SCM，将PMDK pmemobj offset转换成direct memory address;
 * 2. 对于DMA acc，将SPDK blob page offset/scm addr映射到dma buffer，对于fetch操作还要执行写buffer
*/
int
	     ...
	     unsigned int bulk_perm)
{
	...
	biod->bd_rdma = (bulk_ctxt != NULL) || (type == BIO_CHK_TYPE_REBUILD); //simple test没走

	if (bulk_ctxt != NULL && !(daos_io_bypass & IOBP_SRV_BULK_CACHE)) {//simple test没走
	...
	rc = iod_map_iovs(biod, arg); //Convert offset of @biov into memory pointer 
	if (rc)
		return rc;
	...
	if (biod->bd_rsrvd.brd_rg_cnt == 0)
		return 0;
	//
	bdb = iod_dma_buf(biod); //bio_dma_buffer
	bdb->bdb_active_iods++;
	if (bdb->bdb_stats.bds_active_iods)
		d_tm_set_gauge(bdb->bdb_stats.bds_active_iods, bdb->bdb_active_iods);

	if (biod->bd_type < BIO_IOD_TYPE_GETBUF) {
		rc = ABT_eventual_create(0, &biod->bd_dma_done);  //创建abt 事件
		if (rc != ABT_SUCCESS) {
			rc = -DER_NOMEM;
			goto failed;
		}
...
/**
 * 1. 释放DMA BUFFER
 * 2. 将DMA buffer写到NVMe/SCM(在通过DMA访问SCM的情况)
 * 3. 对于direct access SCM该函数相当于无效操作
*/
int
...
bio_iod_copy(struct bio_desc *biod, d_sg_list_t *sgls, unsigned int nr_sgl)
{
	...
	return iterate_biov(biod, copy_one, &arg);//arg是dram的sglist的相关信息
}
