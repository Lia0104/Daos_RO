	   ...
	   bool force)
{
	...
	if (md->vsd_magic == VEA_MAGIC) {
		...
		erase_md(umem, md); //destroy extent tree and vector tree
	}
		    ...
		    d_list_t *resrvd_list, bool publish)
{
	...
	struct vea_free_extent	 vfe; //vfe就是将resrvd ext列表中的extent合起来，一块进行cancel或者publish操作。
	...
	d_list_for_each_entry(resrvd, resrvd_list, vre_link) {
		...
		if (vfe.vfe_blk_off + vfe.vfe_blk_cnt == resrvd->vre_blk_off) {//vfe是即将要free的extent，如果下一个extent也是要free的，将他包含在free extent中。
	...
	if (vfe.vfe_blk_cnt != 0) {
		rc = publish ? persistent_alloc(vsi, &vfe) :
			       compound_free(vsi, &vfe, 0); //往free tree中添加free exent
		if (rc)
			goto error;
	}
...
free_commit_cb(void *data, bool noop)
{
	 ...
	 * persistent free tree中有free extent但是in memory free tree中没有是可以接受的，但是反过来是不可接受的 
	 */
