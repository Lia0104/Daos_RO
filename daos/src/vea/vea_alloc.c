	       ...
	       struct vea_entry *entry)
{
	...
	remain = &entry->ve_ext;//entry是vea_space_info中free tree的一条entry
	...
	free_class_remove(vsi, entry);  //从heap或size tree中删除

	if (remain->vfe_blk_cnt == vfe->vfe_blk_cnt) {
		d_iov_set(&key, &vfe->vfe_blk_off, sizeof(vfe->vfe_blk_off));
		rc = dbtree_delete(vsi->vsi_free_btr, BTR_PROBE_EQ, &key, NULL); //从all free extent tree中删除
	} else {
		/* Adjust in-tree offset & length */
		remain->vfe_blk_off += vfe->vfe_blk_cnt;
		remain->vfe_blk_cnt -= vfe->vfe_blk_cnt;

		rc = free_class_add(vsi, entry); //上面从heap或size tree中remove，这里重新add
	}
	     ...
	     struct vea_resrvd_ext *resrvd)
{
	...
	if (resrvd->vre_hint_off == VEA_HINT_OFF_INVAL) //hint是放在resrvd中
		return 0;
	...
	rc = compound_alloc(vsi, &vfe, entry);  //更新in memory compound index
	if (rc)
		return rc;
	       ...
	       struct vea_resrvd_ext *resrvd)
{
	...
	entry = container_of(root, struct vea_entry, ve_node);//找到root所在的enty的指针
	 ...
	 * largest free extent.
	 */
	if (entry->ve_ext.vfe_blk_cnt <= (max(blk_cnt, vfc->vfc_large_thresh) * 2)) {  //largest free extent不够二分
		...
		rc = compound_alloc(vsi, &vfe, entry); //处理compound index
		if (rc)
			return rc;

	} else {   //  the largest free extent is large enough 
