	      ...
	      unsigned int flags)
{
	...
	rc = dbtree_upsert(vsi->vsi_free_btr, BTR_PROBE_BYPASS, DAOS_INTENT_UPDATE, &key,  //创建transient free tree中的1个extent
			   &val, &val_out);
	if (rc != 0) {
	...
	D_INIT_LIST_HEAD(&entry->ve_link); //只是初始化lru链表
