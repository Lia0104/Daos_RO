		 ...
		 uint32_t cnt)
{
	...
	vfe.vfe_blk_off = off;
	vfe.vfe_blk_cnt = cnt;
	rc = verify_free_entry(NULL, &vfe);//判断vfe中off和cnt是否为空
	if (rc)
		return rc;
...
repeat:
	...
	rc = verify_free_entry(key_off, ext); //判断key_off是否等于ext中的off
	if (rc != 0)
		return rc;

	rc = ext_overlapping(ext, &vfe);//判断分配的extent vfe和现在重新找到ext是否有重叠。如果返回1表示ext包含vfe，说明是Not allocated，对应上面的函数头注释
	if (rc)
		return rc;
