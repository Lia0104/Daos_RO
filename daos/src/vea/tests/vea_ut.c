...
struct vea_ut_args {
	...
	struct vea_space_info		*vua_vsi;  //In-memory compound index 
...
ut_format(void **state)
{
	...
	rc = vea_format(&args->vua_umm, &args->vua_txd, args->vua_md, blk_sz,
			hdr_blks, capacity, NULL, NULL, false);
	assert_rc_equal(rc, -DER_EXIST); //未加force，已经format过
...
//从vea_space_info，即in memory compound index中找stat和attr
static void
ut_query(void **state)
{
	struct vea_ut_args	*args = *state;
...
ut_reserve(void **state)
{
	...
	for (ext_cnt = 0; ext_cnt < 2; ext_cnt++) {
		...
		if (ext_cnt == 0)
			...
			assert_int_equal(ext->vre_blk_off, off_a);//IO stream0的第2次分配是接着IO stream0的第一次分配的末尾

		//transient md中显示已经分配
		rc = vea_verify_alloc(args->vua_vsi, true, off_a, blk_cnt);//off_a和blk_cnt应该是上面reserve的
		assert_rc_equal(rc, 0);
		//persistent md中显示未分配
		...
		if (ext_cnt == 0)
			...
			assert_int_equal(ext->vre_blk_off, off_b);//IO stream1的第2次分配是接着IO stream1的第1次分配的末尾
	...
	assert_int_equal(ext->vre_blk_off, off_b);//IO stream2的第1次分配是接着IO stream1的第2次分配的末尾
...
ut_cancel(void **state)
{
	...
	d_list_for_each_entry(ext, r_list, vre_link) { //遍历IO stream0分配的extent，算出起始偏移量和总数。
	...
	rc = vea_verify_alloc(args->vua_vsi, true, blk_off, blk_cnt);//在transient metadata中并没有分配
	assert_rc_equal(rc, 1);
...
ut_tx_publish(void **state)
{
	...
	for (i = 1; i < IO_STREAM_CNT; i++) {
		r_list = &args->vua_resrvd_list[i]; //reserve list
		h_ctxt = args->vua_hint_ctxt[i]; //hint context

		/*
		 * the reserved list will be freed on publish, save the
		 * allocated extents for later verification.
		 * copy一份新的reserve list只是为了后续的verification
		 */
...
ut_free(void **state)
{
	...
	d_list_for_each_entry(ext, r_list, vre_link) {
		...
		rc = vea_verify_alloc(args->vua_vsi, true, blk_off, blk_cnt); //transient
		assert_rc_equal(rc, 0); //在transient还是被分配状态

		rc = vea_verify_alloc(args->vua_vsi, false, blk_off, blk_cnt); //persist
		assert_rc_equal(rc, 1);//persist md中是已经被free的。
	}

	print_message("transient free extents:\n");
	vea_dump(args->vua_vsi, true); //打印transient md
	...
	d_list_for_each_entry(ext, r_list, vre_link) {
		...
		rc = vea_verify_alloc(args->vua_vsi, true, blk_off, blk_cnt);
		assert_rc_equal(rc, 1); //transient 是未分配的
	}
