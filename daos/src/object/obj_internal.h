...
struct dc_obj_shard {
	/** refcount */
	unsigned int		do_ref;
	uint32_t		do_target_rank;// engine rank?
	/** object id */
	...
	uint32_t		do_shard_idx;//obj中的第do_shard_idx个shard
	uint8_t			do_target_idx;	/* target VOS index in node */ //engine rank下的target
};
...
struct dc_object {
	...
	/** object open mode */
	unsigned int		 cob_mode; //read only/rw等
	unsigned int		 cob_version;
	unsigned int		 cob_shards_nr;  //shard个数
	unsigned int		 cob_grp_size; //shard_cnt
...
struct obj_reasb_req {
	...
	struct obj_io_desc		*orr_oiods; //for EC object
