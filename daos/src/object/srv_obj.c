 ...
 */
static int
obj_gen_dtx_mbs(uint32_t flags, uint32_t *tgt_cnt, struct daos_shard_tgt **p_tgts, //flag的每一位代表某一个标志，见obj_rpc_flags
		struct dtx_memberships **p_mbs)
{
	...
	for (i = 0, j = 0; i < *tgt_cnt; i++) {
		if (tgts[i].st_rank == DAOS_TGT_IGNORE)  //rank of the shard； target can be ignored for IO
			continue;

		mbs->dm_tgts[j++].ddt_id = tgts[i].st_tgt_id; //ddt_id：全局的target id
	}
		  ...
		  d_sg_list_t *sgl, int sgl_idx, struct obj_bulk_args *p_arg)
{
	...
	while (iov_idx < sgl->sg_nr_out) {
		...
		local_bulk = vos_iod_bulk_at(ioh, sgl_idx, iov_idx, &local_off); //Get the bulk handle associated with a given I/O descriptor.
		if (local_bulk != NULL) {
			...
			while (iov_idx < sgl->sg_nr_out &&
			       ...
			       vos_iod_bulk_at(ioh, sgl_idx, iov_idx,
						&tmp_off) == local_bulk) {    //多个IOV共享一个bulk handle
				D_ASSERT(tmp_off == 0);
				length += sgl->sg_iovs[iov_idx].iov_len;
				iov_idx++;
			};
			...
			while (iov_idx < sgl->sg_nr_out &&
			       sgl->sg_iovs[iov_idx].iov_buf != NULL &&
			       vos_iod_bulk_at(ioh, sgl_idx, iov_idx,
						&local_off) == NULL) {    //bulk handle为NULL
				length += sgl->sg_iovs[iov_idx].iov_len;
				iov_idx++;
			...
			local_off = 0;
			sgl_sent.sg_nr = sgl_sent.sg_nr_out = iov_idx - start;//计算sg number
		...
		if (bulk_bind)
			...
			rc = crt_bulk_transfer(&bulk_desc,      //Start a bulk transferring (inside an RPC handler).
				cached_bulk ? cached_bulk_cp : bulk_cp, p_arg,
				&bulk_opid);
		if (rc < 0) {
		  ...
		  struct obj_bulk_args *p_arg, struct ds_cont_hdl *coh)
{
	...
	for (i = 0; i < sgl_nr; i++) {
		...
		if (sgls != NULL) {
			...
			bsgl = vos_iod_sgl_at(ioh, i);  //Get the scatter/gather list associated with a given I/O descriptor.
			D_ASSERT(bsgl != NULL);
...
//shadow list包含的extent数据不会在fetch操作中被返回，仅用于EC degraded fetch。
static int
		      ...
		      uint64_t *split_offs, struct dtx_handle *dth)
{
	...
	daos_handle_t			ioh = DAOS_HDL_INVAL;
	struct bio_desc			*biod;//bio
	daos_key_t			*dkey;
	crt_bulk_op_t			bulk_op;
	bool				rma;
	bool				bulk_bind;
	bool				create_map;
	bool				spec_fetch = false; //不走rma的时候为true
	...
	create_map = orw->orw_flags & ORF_CREATE_MAP;  //是否包含daos_iom_t

	iods = split_iods == NULL ? orw->orw_iod_array.oia_iods : split_iods;//这里split_iods为空
	offs = split_offs == NULL ? orw->orw_iod_array.oia_offs : split_offs;
	iod_csums = split_csums == NULL ? orw->orw_iod_array.oia_iod_csums :
					 split_csums;

	//echo类：数据只经过网络栈，并不实际落盘
	if (daos_obj_is_echo(orw->orw_oid.id_pub) ||   //echo类
	    (daos_io_bypass & IOBP_TARGET)) {
		obj_echo_rw(rpc, split_iods, split_offs);  // echo的rw
		D_GOTO(out, rc = 0);
	}
	...
	if (obj_rpc_is_update(rpc)) {
		...
		rc = vos_update_begin(ioc->ioc_vos_coh, orw->orw_oid,     //VOS层  预留空间
		...
		bool				 ec_deg_fetch;//降级获取
		bool				 ec_recov; //数据恢复
		bool				 is_parity_shard;
		struct daos_recx_ep_list	*shadows = NULL;

		bulk_op = CRT_BULK_PUT; //两个方向
		if (!rma && orw->orw_sgls.ca_arrays == NULL) {  //ca_array?
			spec_fetch = true; //不走rma的时候为true
			if (orw->orw_flags & ORF_CHECK_EXISTENCE)
				fetch_flags = VOS_OF_FETCH_CHECK_EXISTENCE;
			else
				fetch_flags = VOS_OF_FETCH_SIZE_ONLY; //query iod size有sm作用？
		}
		...
		if (ec_recov) {
			...
			is_parity_shard = (orw->orw_oid.id_shard % obj_ec_tgt_nr(&ioc->ioc_oca)) >=  //是否有冗余的分片：parity shard
		...
		if (ec_deg_fetch && !spec_fetch) {  //这个if分支的作用？？？  !spec_fetch约等于走rma
			if (orwo->orw_rels.ca_arrays != NULL) { //这是orwo，out的，清空orwo的ca_array
				/* Re-entry case */
				daos_recx_ep_list_free(orwo->orw_rels.ca_arrays,
						       orwo->orw_rels.ca_count);
			 ...
			 */
			rc = daos_iod_recx_dup(iods, orw->orw_nr, &iods_dup/*指针的指针*/);
			if (rc != 0) {
			...
			rc = obj_fetch_shadow(ioc, orw->orw_oid,   //shandow指的是什么？？？
					      orw->orw_epoch, cond_flags, dkey,
					      orw->orw_nr, iods,
		...
		if (rma) {
			orwo->orw_sgls.ca_count = 0;
			orwo->orw_sgls.ca_arrays = NULL;  //sgls是用于非rma的数据传输的，这里不用
	...
	biod = vos_ioh2desc(ioh);//biod应该是由SG list
	//对于fetch操作还要执行写buffer
	rc = bio_iod_prep(biod, BIO_CHK_TYPE_IO, rma ? rpc->cr_ctx : NULL,  //Prepare all the SG lists of an io descriptor.
			  CRT_BULK_RW);
	if (rc) {
	...
	if (rc == -DER_CSUM)
		obj_log_csum_err();
post:
	rc = bio_iod_post(biod, rc); //和bio_iod_prep相反，在update下需要将dram buffer中的数据dma写到磁盘
out:
	...
	if (rc == 0 && obj_rpc_is_update(rpc) && dth->dth_need_validation &&
	    sched_cur_seq() != sched_seq) {   //发生了线程切换？
	...
	rc = obj_rw_complete(rpc, ioc, ioh, rc, dth); //已经完成了，处理reply
	if (iods_dup != NULL)
...
{
	struct obj_rw_in		*orw = crt_req_get(rpc); //input parameter struct 
	struct obj_rw_out		*orwo = crt_reply_get(rpc); //output parameter struct 
	...
	rc = obj_ioc_begin(orw->orw_oid.id_pub, orw->orw_map_ver, //检查
			   orw->orw_pool_uuid, orw->orw_co_hdl,
			   orw->orw_co_uuid, opc_get(rpc->cr_opc),
			   orw->orw_flags, &ioc);
	if (rc != 0) {
	...
	if (obj_rpc_is_fetch(rpc)) {
		...
		rc = dtx_begin(ioc.ioc_vos_coh, &orw->orw_dti, &epoch, 0, orw->orw_map_ver,  //Prepare the DTX handle in DRAM.
			       &orw->orw_oid/*leader_oid*/, NULL/*dti_cos*/, 0, dtx_flags, NULL, &dth);
		if (rc == 0) {
			rc = obj_local_rw(rpc, &ioc, NULL, NULL, NULL, dth);
			rc = dtx_end(dth, ioc.ioc_coc, rc);
		}
...
again2:
	...
	rc = dtx_leader_begin(ioc.ioc_vos_coh, &orw->orw_dti, &epoch, 1,  //Prepare the leader DTX handle in DRAM.
	...
	rc = dtx_leader_exec_ops(dlh, obj_tgt_update, NULL, NULL, &exec_arg);  //Execute the operations on all targets.
