...
struct obj_req_tgts {
	...
	uint32_t		 ort_srv_disp:1; //within each group the first target is the leader shard 
};
...
open_retry:
	if (obj->cob_version != map_ver) {
		...
		D_GOTO(unlock, rc = -DER_STALE); //数据过时
	}

	obj_shard = &obj->cob_shards->do_shards[shard];//只是获得指针，此时obj_shard中还没有东西

	/* Skip the invalid shards and targets */
	if (obj_shard->do_shard == -1 || // shard identifier 
	    obj_shard->do_target_id == -1) { // target id 
	...
	if (obj_shard->do_obj == NULL) {
		daos_unit_oid_t	 oid; //object-shard id
...
obj_layout_create(struct dc_object *obj, unsigned int mode, bool refresh)
{
	...
	map = pl_map_find(pool->dp_pool, obj->cob_md.omd_id); //通过objcet id在pool中找到对应的pl_map
	if (map == NULL) {
	...
	dc_pool_put(pool);
	rc = pl_obj_place(map/*pl_map*/, &obj->cob_md, mode, NULL, &layout);
	pl_map_decref(map);
	if (rc != 0) {
	...
	obj->cob_shards_nr = layout->ol_nr; //shard 个数
	obj->cob_grp_size = layout->ol_grp_size; // group中shard个数
	old = obj->cob_grp_nr;
	obj->cob_grp_nr = obj->cob_shards_nr / obj->cob_grp_size;//group个数
	...
	for (i = 0; i < layout->ol_nr; i++) {
		...
		obj_shard->do_shard/*pl_obj_shard*/ = layout->ol_shards[i].po_shard;
...
obj_replica_leader_select(struct dc_object *obj, unsigned int grp_idx)
{
	...
	D_ASSERT(oca != NULL);
	grp_size = obj_get_grp_size(obj);
	if (grp_size == 1) {  //only one replication
		   ...
		   bool cond_modify, uint8_t *bit_map)
{
	...
	if (obj_is_ec(obj))
		...
		rc = obj_replica_leader_select(obj, grp_idx); //return pos（int）
...
obj_dkey2grpidx(struct dc_object *obj, uint64_t hash, unsigned int map_ver)
{
	...
	grp_size = obj_get_grp_size(obj);//group size就存在dc_object中
	D_ASSERT(grp_size > 0);
	...
	if (obj->cob_version != map_ver || map_ver < pool_map_ver) {
		...
		dc_pool_put(pool);
		return -DER_STALE; //数据过时错误
	}
	dc_pool_put(pool);

	D_ASSERT(obj->cob_shards_nr >= grp_size);

	grp_idx = d_hash_jump(hash, obj->cob_shards_nr / grp_size); //bucket = shards_nr / group_size
	D_RWLOCK_UNLOCK(&obj->cob_lock);
		 ...
		 uint32_t *start_shard, uint32_t *grp_size)
{
	...
	grp_idx = obj_dkey2grpidx(obj, hash, map_ver); //consistent jump hash 算法
	if (grp_idx < 0)
		return grp_idx;
 ...
 */
static int
obj_shard_tgts_query(struct dc_object *obj, uint32_t map_ver, uint32_t shard/*start shard*/,
		     uint16_t ec_tgt_idx, struct daos_shard_tgt *shard_tgt/*shard_tgt数组中要放置新shard_tgt的某个slot*/,
...
shard_open:
	rc = obj_shard_open(obj, shard, map_ver, &obj_shard/*[out]*/);
	if (unlikely(DAOS_FAIL_CHECK(DAOS_FAIL_SHARD_OPEN) &&  //故障注入，用于测试？
...
ec_deg_get:
	...
	shard_tgt->st_shard	= shard; /* shard index */
...
static int
obj_shards_2_fwtgts(struct dc_object *obj, uint32_t map_ver, uint8_t *bit_map/*target bitmap??*/,
		    uint32_t start_shard, uint32_t shard_cnt/*grp size*/, uint32_t grp_nr/*==1 when update*/,
		    uint32_t flags, struct obj_auxi_args *obj_auxi)
{
	struct obj_req_tgts	*req_tgts = &obj_auxi->req_tgts; //此时应该还是空
	struct daos_shard_tgt	*tgt = NULL;
	...
	if (cli_disp || bit_map != NIL_BITMAP)
		D_ASSERT(grp_nr == 1);
	req_tgts->ort_start_shard = start_shard;//for ec obj
	req_tgts->ort_srv_disp = (srv_io_mode != DIM_CLIENT_DISPATCH) &&
				  !cli_disp && grp_size > 1;
	//shard_nr
	if (bit_map != NIL_BITMAP) { //target bit map已经不为空了
		shard_nr = 0;
		for (i = 0; i < shard_cnt; i++)
			if (isset(bit_map, i)) //判断位图中某一位是否被置位
				shard_nr++;
	} else {
		shard_nr = shard_cnt;
	}

	if (shard_nr > OBJ_TGT_INLINE_NR) {   //表示ort_tgts_inline[OBJ_TGT_INLINE_NR]不够放
		//1. 这个if清空req_tgts->ort_shard_tgts 
		...
		//2. 这个if重新分配req_tgts->ort_shard_tgts，个数为shard_nr
		if (req_tgts->ort_shard_tgts == NULL) {
		...
		//shard_nr <= OBJ_TGT_INLINE_NR时，将req_tgts->ort_shard_tgts指针指向ort_tgts_inline
		if (req_tgts->ort_shard_tgts != NULL &&
		    req_tgts->ort_shard_tgts != req_tgts->ort_tgts_inline)  //ort_shard_tgts不指向inline tgt数组
			D_FREE(req_tgts->ort_shard_tgts);
	...
	if (obj_auxi->spec_shard) {
		D_ASSERT(grp_nr == 1);//只有1个group
		D_ASSERT(shard_cnt == 1);//只有1个shard
		...
		rc = obj_shard_tgts_query(obj, map_ver, start_shard,
					  start_shard % obj_get_grp_size(obj)/*ec target index*/,
	...
	for (i = 0; i < grp_nr; i++) { 
		...
		shard_idx = start_shard + i * grp_size;//每个group的start shard
		head = tgt = req_tgts->ort_shard_tgts + i * grp_size; //第i个group的第一个daos_shard_tgt
		//搜索leader shard的target
		if (req_tgts->ort_srv_disp) {
			if (obj_auxi->opc == DAOS_OBJ_RPC_UPDATE &&
			    DAOS_FAIL_CHECK(DAOS_DTX_SPEC_LEADER))
				rc = 0;
			else
				rc = obj_grp_leader_get(obj, shard_idx / grp_size, map_ver, //query leader
			...
			if (rc < 0) {
				D_ERROR(DF_OID" no valid shard %u, grp size %u "
					...
					obj_get_replicas(obj)/*replicas num*/,
					obj_is_ec(obj) ? "EC-obj" : "REP-obj",
					DP_RC(rc));
				return rc;
			}
			leader_shard = rc;
			rc = obj_shard_tgts_query(obj, map_ver, leader_shard, //query shard targets
						  0, tgt++/*数组，指针*/, obj_auxi);
			if (rc != 0)
				return rc;
		...
		//遍历1个group下的所有shard，query shard对应的tgt，放在req_tgts->ort_shard_tgts数组里
		for (j = 0, fail_nr = 0; j < grp_size; j++, shard_idx++) {
			...
			rc = obj_shard_tgts_query(obj, map_ver, shard_idx,
						  shard_idx - start_shard/*ec_tgt_idx*/,
						  tgt++, obj_auxi);
...
dc_obj_open(tse_task_t *task)
{
	...
	//下面这些函数都是本地操作
	rc = dc_obj_fetch_md(args->oid, &obj->cob_md); //从
	if (rc != 0)
		D_GOTO(fail_rwlock_created, rc);

	rc = obj_init_oca(obj); //obj class attr实际上是放在obj id里的
	if (rc != 0)
		D_GOTO(fail_rwlock_created, rc);

	rc = obj_layout_create(obj, obj->cob_mode, false);//根据pl_map和obj id计算出layout，封装成dc_objcet
	if (rc != 0)
		D_GOTO(fail_rwlock_created, rc);
	      ...
	      crt_bulk_t **p_bulks)
{
	...
	for (; sgls != NULL && i < nr; i++) {
		if (sgls[i].sg_iovs != NULL &&
			...
			rc = crt_bulk_create(daos_task2ctx(task), &sgls[i], // mercury
					     bulk_perm, &bulks[i]);
			if (rc < 0)
				D_GOTO(out, rc);
			if (!bulk_bind)
				continue;
			rc = crt_bulk_bind(bulks[i], daos_task2ctx(task));  //将buld handle与task的上下文相绑定，bulk handle中有client端数据的原始地址
			if (rc != 0)
				D_GOTO(out, rc);
		}
	}
...
obj_req_get_tgts(struct dc_object *obj, int *shard, daos_key_t *dkey,
		 uint64_t dkey_hash, uint8_t *bit_map/*应该还没置位*/, uint32_t map_ver,
		 bool to_leader, bool spec_shard, struct obj_auxi_args *obj_auxi)
{
		...
		if (bit_map == NIL_BITMAP && !is_ec_check_exist) { // replica obj走这
			if (spec_shard) {
				...
				if (DAOS_FAIL_CHECK(DAOS_DTX_RESYNC_DELAY))
					...
					rc = obj_dkey2shard(obj, dkey_hash, map_ver, to_leader, //dkey->leader shard for replication case or to valid shards for ec case
							    obj_auxi->failed_tgt_list);
				if (rc < 0)
					goto out;
			}
			shard_idx = rc;//leader shard index or valid shard index
			shard_cnt = 1;
		} else {
	...
	case DAOS_OBJ_RPC_PUNCH_AKEYS:
		grp_nr = 1;
		rc = obj_dkey2grpmemb(obj, dkey_hash, map_ver, &shard_idx/*start shard*/, &shard_cnt/*grp size*/);
		if (rc != 0)
			goto out;
	...
	//shards: shard_idx开始shard_cnt个shard
	rc = obj_shards_2_fwtgts(obj, map_ver, bit_map, shard_idx/*start shard*/,
				 shard_cnt/*group size*/, grp_nr/*==1*/, flags, obj_auxi/*[out]*/);
	if (rc != 0) {
...
{
	struct obj_req_tgts	*req_tgts = &obj_auxi->req_tgts;
	d_list_t		*task_list = &obj_auxi->shard_task_head;//shard task list
	...
	tgt = req_tgts->ort_shard_tgts;
	tgts_nr = req_tgts->ort_srv_disp ? req_tgts->ort_grp_nr : //server dispatch只需要发到group个leader server上，否则需要到所有的leader上
	...
	if (tgts_nr == 1 && !require_shard_task) {
		...
		shard_auxi->shard_io_cb = io_cb;
		rc = io_prep_cb(shard_auxi, obj, obj_auxi, dkey_hash,  //shard_rw_prep()
				shard_auxi->grp_idx);
		if (rc)
			goto out_task;
	...
	for (i = 0; i < tgts_nr; i++) {
		...
		shard_auxi->obj_auxi = obj_auxi;
		shard_auxi->shard_io_cb = io_cb;
		rc = io_prep_cb(shard_auxi, obj, obj_auxi, dkey_hash,   //prep callback函数
				shard_auxi->grp_idx);
		if (rc) {
		...
		tse_task_list_add(shard_task, task_list); //shard task list
next:
		if (req_tgts->ort_srv_disp)
...
task_sched:
	if (!d_list_empty(&obj_auxi->shard_task_head))  //调度schedule shard task
	      ...
	      uint32_t grp_idx)
{
	...
	if (daos_handle_is_inval(obj_auxi->th))
		...
		dc_tx_get_dti(obj_auxi->th, &shard_arg->dti/*dtx id*/);
...
dc_obj_fetch_task(tse_task_t *task)
{
	...
	rc = obj_task_init(task, DAOS_OBJ_RPC_FETCH, map_ver, args->th,   //这里连接server
			   &obj_auxi, obj);
	if (rc != 0) {
		obj_decref(obj);
		D_GOTO(out_task, rc);
	      ...
	      daos_obj_update_t *args, struct dc_object *obj)
{
	...
	rc = obj_task_init(task, DAOS_OBJ_RPC_UPDATE, map_ver, args->th,   //这里连接server
			   &obj_auxi, obj);
	if (rc != 0) {
		obj_decref(obj);
		D_GOTO(out_task, rc);
	...
	rc = obj_rw_req_reassemb(obj, args, NULL, obj_auxi); //reorder or ,erge adjacent recxs
	if (rc) {
		D_ERROR(DF_OID" obj_req_reassemb failed %d.\n",
			DP_OID(obj->cob_md.omd_id), rc);
		D_GOTO(out_task, rc);
	...
	rc = obj_csum_update(obj, args, obj_auxi);  //checksum放在auxi中
	if (rc) {
	...
	rc = obj_req_fanout(obj, obj_auxi, dkey_hash, map_ver, epoch,  //向多个tgt上写数据
			    shard_rw_prep, dc_obj_shard_rw, task);
	return rc;
...
dc_obj_update_task(tse_task_t *task)
{
	...
	rc = obj_req_valid(task, args, DAOS_OBJ_RPC_UPDATE, &epoch, &map_ver, //这里将arg->object handle转换为dc_object类型
			   &obj);
	if (rc != 0)
		goto comp;
...
dc_obj_list_dkey(tse_task_t *task)
{
	...
	return obj_list_common(task, DAOS_OBJ_DKEY_RPC_ENUMERATE, args);  //连接server
}
...
obj_ec_parity_alive(daos_handle_t oh, uint64_t dkey_hash, uint32_t *shard)
{
	...
	oca = obj_get_oca(obj);
	p_shard = obj_ec_data_tgt_nr(oca);
